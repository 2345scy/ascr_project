		...
		if(n_i != 0){
			for(int i = 1; i <= n_i; i++){
				...
				for(int m = 1; m <= n_m; m++){
					...
					Type fy_toa_log = Type(0.0);
					Type fy_bear_log = Type(0.0);
					Type fy_dist_log = Type(0.0);
					Type fy_ss_log = Type(0.0);
					...
					if(is_bearing == 1){
						for(int t = 1; t <= n_t; t++){
							...
							fy_bear_log += (kappa_tem * cos(capt_bearing(index_data_full) - theta(index_data_dist_theta)) - 
								log(besselI(kappa_tem, Type(0)))) * pow(0, 1 - capt_bin(index_data_full));
						}
					}
					...
					if(is_dist == 1){
						for(int t = 1; t <= n_t; t++){
							...
							fy_dist_log +=  ((-1) * (alpha_tem * (log(dx(index_data_dist_theta)) - log(alpha_tem)) + lgamma(alpha_tem)) + (alpha_tem - 1) * 
								log(capt_dist(index_data_full)) - alpha_tem * capt_dist(index_data_full) / dx(index_data_dist_theta)) * pow(0, 1 - capt_bin(index_data_full));
						}
					}
					...
					if(is_ss_origin == 1){
						for(int t = 1; t <= n_t; t++){
							...
							fy_ss_log += dnorm(capt_ss(index_data_full), mu_tem, sigma_ss_tem, true) * pow(0, 1 - capt_bin(index_data_full));
						}
					}
